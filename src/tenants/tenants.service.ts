...
export class TenantsService {
  ...
  async create(dto: CreateTenantDto): Promise<{ tenant: Tenant; apiKey: string }> {
    const existing = await this.tenantRepo.findOne({ where: [{ name: dto.name }, { slug: dto.slug }] });
    if (existing) throw new ConflictException('Tenant with this name or slug already exists');

    const rawApiKey = `ffs_${uuidv4().replace(/-/g, '')}`;
    const apiKeyHash = await bcrypt.hash(rawApiKey, 12);

    const tenant = this.tenantRepo.create({ ...dto, apiKeyHash });
