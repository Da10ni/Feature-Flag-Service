...
import {
  FlagEnvironment,
  Environment,
} from './entities/flag-environment.entity';
...
export class FlagsService {
  ...
  private async invalidateCache(tenantId: string): Promise<void> {
    await Promise.all(
      Object.values(Environment).map((env) =>
        this.redisService.del(`flags:${tenantId}:${env}`),
      ),
    );
  }
  ...
  async create(
    ...
    const existing = await this.flagRepo.findOne({
      where: { tenantId, flagKey: dto.flagKey },
    });
    if (existing)
      throw new ConflictException(
        `Flag with key '${dto.flagKey}' already exists`,
      );
  ...
  async findAll(
    ...
    if (environment)
      qb.andWhere('env.environment = :environment', { environment });
  ...
  async update(
    ...
    if (
      ...
      const envRecord = flag.environments.find(
        (e) => e.environment === dto.environment,
      );
      if (envRecord) {
        if (dto.isEnabled !== undefined) envRecord.isEnabled = dto.isEnabled;
        if (dto.rolloutPercentage !== undefined)
          envRecord.rolloutPercentage = dto.rolloutPercentage;
